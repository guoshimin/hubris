...
fn main() -> ! {
    ...
    let mut socket_storage = [smoltcp::iface::SocketStorage::EMPTY; generated::SOCKET_COUNT];
    let mut eth = smoltcp::iface::InterfaceBuilder::new(eth, &mut socket_storage[..])
    ...
    for (&h, &port) in socket_handles.iter().zip(&generated::SOCKET_PORTS) {
        eth.get_socket::<UdpSocket>(h).bind(port)
            .map_err(|_| ()).unwrap();
    }
    ...
    let mut server = server::ServerImpl::new(
        socket_handles,
        eth,
    );
    ...
    loop {
        ...
        // Call into smoltcp.
        let poll_result = server.interface_mut().poll(
