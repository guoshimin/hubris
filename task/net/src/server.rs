...
use idol_runtime::{RequestError, ClientError, NotificationHandler};
...
impl<'a> ServerImpl<'a> {
    ...
    fn get_handle(&self, index: usize) -> Result<SocketHandle, RequestError<NetError>> {
        self.socket_handles.get(index)
            .cloned()
    ...
    pub fn get_socket_mut(&mut self, index: usize) -> Result<&mut UdpSocket<'a>, RequestError<NetError>> {
...
impl idl::InOrderNetImpl for ServerImpl<'_> {
    ...
    fn recv_packet(
        ...
        if generated::SOCKET_OWNERS[socket_index].0.index() != msg.sender.index() {
        ...
        match socket.recv() {
            Ok((body, endp)) => {
                ...
                payload.write_range(0..body.len(), body)
                    .map_err(|_| RequestError::went_away())?;

                Ok(UdpMetadata {
                    port: endp.port,
                    size: body.len() as u32,
                    addr: endp.addr.try_into()
                        .map_err(|_| ())
                        .unwrap(),
                })
            }
    ...
    fn send_packet(
        ...
        match socket.send(payload.len(), metadata.into()) {
            Ok(buf) => {
                payload.read_range(0..payload.len(), buf)
                    .map_err(|_| RequestError::went_away())?;
                Ok(())
            }
            Err(smoltcp::Error::Exhausted) => {
...
mod idl {
    use task_net_api::{UdpMetadata, NetError};
